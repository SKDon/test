...
//using AirWaybillModel = Alicargo.ViewModels.AirWaybillModel;
...
//		private void SetStateSetup(long AirWaybillId, AirWaybillModel airWaybillModel, ApplicationData[] applicationDatas,
//			long stateId, IEnumerable<long> ids)
//		{
//			_context.AirWaybillRepository.Setup(x => x.Get(AirWaybillId)).Returns(new[] { airWaybillModel });
...
//			var AirWaybillData = _context.Build<AirWaybillModel>().With(x => x.Id, AirWaybillId).Create();
...
//			var model = _context.Create<ViewModels.AirWaybillEditModel>();
...
//			SetupUnitOfWork();
//			_context.AirWaybillRepository.Setup(x => x.Get(AirWaybillId)).Returns(new[] { (AirWaybillModel)model });
//			_context.AirWaybillRepository.Setup(x => x.Update(model, model.GTDFile,
//				model.GTDAdditionalFile, model.PackingFile, model.InvoiceFile, model.AWBFile));
