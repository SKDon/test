	...
	$a.Calculation = (function ($c) {
		$c.DataSource = function () {
			...
			return {
				...
				error: $a.ShowError,
				group: {
					field: "AwbDisplay", dir: "asc", aggregates: [
		...
		$c.Columns = function () {
			var c = [
				...
				{ field: "TotalTariffCost", title: $l.Entities_TotalTariffCost, groupFooterTemplate: "#= kendo.toString(sum, 'n2') #" },
				...
				{ field: "Profit", title: $l.Entities_Profit, groupFooterTemplate: "#= kendo.toString(sum, 'n2') #" }
			];
			if ($r.IsAdmin) {
				c.push({
					command: [{
